...
public class App {
	public static void main(String[] args) throws InterruptedException {
		ProcessEngineConfiguration cfg = new StandaloneProcessEngineConfiguration()
				...
				.setJdbcDriver("org.h2.Driver").setJobExecutorActivate(true)
		...
		// 流程部署
		Deployment deployment = repositoryService.createDeployment().addClasspathResource("MultiTask.bpmn").name("流程测试")
				.category("").deploy();
		...
		// 部门主任
		List<Task> taskListDept1 = taskService.createTaskQuery().taskAssignee("dept").orderByTaskCreateTime().desc()
				.list();
		...
		// 市场专员
		List<Task> taskListSczy = taskService.createTaskQuery().taskAssignee("sczy").orderByTaskCreateTime().desc()
				.list();
		...
		// 市场主任
		List<Task> taskListSczr = taskService.createTaskQuery().taskAssignee("sczr").orderByTaskCreateTime().desc()
				.list();
		...
		// 财务专员
		List<Task> taskListCwzy = taskService.createTaskQuery().taskAssignee("cwzy").orderByTaskCreateTime().desc()
				.list();
		...
		// 财务主任
		List<Task> taskListCwzr = taskService.createTaskQuery().taskAssignee("cwzr").orderByTaskCreateTime().desc()
				.list();
		...
		// 技术专员审批
		List<Task> taskListJishu = taskService.createTaskQuery().taskAssignee("jishu").orderByTaskCreateTime().desc()
				.list();
		System.out.println("taskListJishu = " + taskListJishu);
		taskService.complete(taskListJishu.get(0).getId());

		// 综合部公示
		List<Task> taskListZonghe = taskService.createTaskQuery().taskAssignee("zhb").orderByTaskCreateTime().desc()
				.list();
		System.out.println("taskListZonghe = " + taskListZonghe);
		taskService.complete(taskListZonghe.get(0).getId());

		// 公示确认，5秒未完成自动进入下一流程
		List<Task> taskListPublic = taskService.createTaskQuery().taskAssignee("张三").orderByTaskCreateTime().desc()
				.list();
		...
		// 分管领导确认
		List<Task> taskListLeader = taskService.createTaskQuery().taskAssignee("leader").orderByTaskCreateTime().desc()
				.list();
